	...
	private void ifColliderIsPaddle() {
		if (collider == rect) { //left and right sides
			if (collider == getElementAt(ball.getX() - 1, ball.getY() + BALL_RADIUS)) {
				if (vy > 0) {
					if(vx < 0){    //if vx > 0 and ball is hit from the left side, it should not change
						vx = -vx;         // vx speed
					}
					vy = -vy;
				}
			} else if (collider == getElementAt(ball.getX() + 2 * BALL_RADIUS + 1, ball.getY() + BALL_RADIUS)){
				if (vy > 0) {
					if(vx > 0){     //if vx < 0 and ball is hit from the right side, it should not
						vx = -vx;           // change vx speed
					}
					vy = -vy;
				}
			} 
